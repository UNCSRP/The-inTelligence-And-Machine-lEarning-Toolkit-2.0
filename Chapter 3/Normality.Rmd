...
Our example dataset contains subject IDs `S_ID`, subject ages, and measurements of 8 different phthalates from silicone wristbands (`DEP`, `DBP`, `BBP`, `DEHA`, `DEHP`, `DEHT`, `DINP`, and `TOTM`). The units for the chemical data are nanogram of chemical per gram of silicone wristband (ng/g) per day the participant wore the wristband. NOT SURE IF WE NEED A BETTER SEGUEWAY
...
IT'S UP TO YOU IF YOU WANT TO INCLUDE THIS QUESTION, BUT I THOUGHT IT MIGHT BE HELPFUL TO INTRODUCE STATISTICAL TESTS THAT WOULD BE CHOSEN BASED ON THE DATA' DISTRIBUTION
...
```
This test resulted in a p-value of 0.8143, so we cannot reject the null hypothesis (that data are normally distributed). This means that we can assume that these age is normally distributed, which is consistent with our visualizations above. 
...
*IT'S WORTH MENTIONING THAT THE SCALE() FUNCTION IS ALSO USED FOR DATA NORMALIZATION, BUT I WOULD START WITH WHAT DATA NORMALIZATION IS AND WHAT TECHNIQUES ARE EMPLOYED TO NORMALIZE DATA. NOT SURE IF IT'S ALSO WORTH MENTIONING THE DIFFERENCE BETWEEN NORMALIZATION AND STANDARDIZATION*
...
*NOT SURE IF WE WANT TO INCLUDE THIS?* I WAS THINKING THAT TOO, BUT I THINK IT'S BETTER SUITED FOR TESTS FOR STATISTICAL DIFFERENCES AS OPPOSED TO NORMALITY
...
ADD OTHER LINKS TO NORMALITY RESOURCES

## Test Your Knowledge

*DO WE WANT TYK FOR THIS?* YES! WE CAN JUST HAVE THEM TEST FOR NORMALITY OF A DIFFERENT DATASET
