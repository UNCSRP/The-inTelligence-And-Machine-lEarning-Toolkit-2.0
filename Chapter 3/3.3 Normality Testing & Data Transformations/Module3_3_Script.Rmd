...
```{r message = FALSE}
# Import data
wrist_data <- read.xlsx("Module3_3_InputData.xlsx")
...
knitr::include_graphics("Module3_3_Images/Module3_3_Image1.png")
```

Common parametric statistical tests, such as t-tests and one-way ANOVAs, rely on the assumption that the data fall within the normal distribution for calculation of z-scores and p-values. Non-parametric tests, such as the Wilcoxon Rank Sum test and the Kruskal-Wallis test, do not rely on assumptions about data distribution. Some of the aforementioned between-group comparisons were introduced in [TAME 2.0 Module 3.4 Introduction to Statistical Tests](insert link). They along with non-parametric tests are explored further in later modules including [TAME 2.0 Module 4.4 Two-Group Comparisons & Visualizations](insert link) and [TAME 2.0 Module 4.5 Multi-group Comparisons & Visualizations](insert link).  
...
The `hist()` function is useful for plotting single distributions, but what if we have many variables that need normality assessment? We can leverage *ggplot2*'s powerful and flexible graphics functions such as `geom_histogram()` and `facet_wrap()` to inspect histograms of all of our variables in one figure panel. For more information on data manipulation in general, see [TAME 2.0 Module 2.3 Data Manipulation & Reshaping](insert link) and for more on *ggplot2* including the use of `facet_wrap()`, see [TAME 2.0 Module 3.2 Improving Data Visualizations](insert link here). 
...
Let's log~2~ transform our chemical data and examine the resulting histograms and Q-Q plots to qualitatively assess whether the data appear more normal following transformation. We will apply a pseudo-log~2~ transformation, where we will add 1 to each value before log~2~ transforming so that all resulting values are positive and any zeroes in the dataframe do not return -Inf. 
```{r fig.align = 'center'}
...
Both the histograms and the Q-Q plots demonstrate that our log~2~ transformed data are more normally distributed than the raw data graphed above. Let's apply the Shapiro-Wilk test to our log~2~ transformed data to see if the chemical distributions are normally distributed.
```{r}
# Apply Shapiro Wilk test
...
3. What are the average Kruskal-Wallis p-values for the raw and psuedo log~2~ transformed data?
