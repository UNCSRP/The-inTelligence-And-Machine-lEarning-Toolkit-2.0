...
This training module was developed by Alexis Payton, MS, Oyemwenosa N. Avenbuan, Lauren Koval, and Dr. Julia E. Rager
...
Let's imagine you're interested in predicting an animal's species (either a cat or a dog) based on a dataset that contains variables regarding weight, height, coat color, ear shape, etc. The most basic way to train a model is dividng these data into a training set and a test set. The **training set** is a subset of the data that the model will learn from to make associations between the predictor variables or features (ie. height, weight etc.) and the outcome (ie. cat or dog). The **test set** is used to evaluate what the model has learned from the training set. 
...
In addition to the training and testing sets, a validation set can also be created for a total of three main sets. Leveraging a validation set is a highly recommended as it helps train the algorithm in a robost way. The training set, validation set, and the test set are detailed further here:

1. **Training Set:** a subset of the data that the algorithm uses to learn from the data by identifying patterns.
...
2. **Test Set:** a subset of data that is used to evaluate the final model's fit based on the training and validation sets. This is akin to the model's final exam, as it provides an objective assessment of its ability to generalize to new data. 

It is important to note that the test set should only be used once, after the algorithm has been trained using the training dataset. Using the test set multiple times during the development process can lead to overfitting, where the model performs well on the test data but poorly on new data. The ideal algorithm is generalizable or flexible enough to accurately predict unseen data. This is known as the bias-variance tradeoff. For further information on the bias-variance tradeoff, see [Understanding the Bias-Variance Tradeoff](https://towardsdatascience.com/understanding-the-bias-variance-tradeoff-165e6942b229).
...
It works by equally splitting the samples in the dataset into *k* number of folds or groups. For example, if 5-fold CV were to be run, we would have 5 different groups in the dataset with 4 retained for validation sets to train the model and 1 retained for testing to fine tune the model's parameters. Across the five iterations, each fold would have a chance to be the test set as seen in the figure below. To measure predictive ability of each of the pararmeters tested, out of bag (OOB) errors are calculated each time. The parameters that yield that the lowest OOB error are used in the algorithm's training on the entire training set. 
...
+ True positive: the number of correctly classified "positive" data points (ie. the number of correctly classified dogs)
+ True negative: the number of correctly classified "negative" data points (ie. the number of correctly classified cats)
+ False positive: the number of incorrectly classified "positive" data points (ie. the number of cats incorrectly classified as dogs)
+ False negative: the number of incorrectly classified "negative" data points (ie. the number of dogs incorrectly classified as cats)
...
+ **Balanced Accuracy:** is the ratio of correct predictions to the total number of predictions or (sensitivity + specificity)/2 and is typically used to assess overall model performance.   

+ **Sensitivity or Recall:** evaluates how well the model was able to predict the "positive" class. It's the ratio of correctly classified true positives to total number of all true positives or TP/(TP + FN). 

+ **Specificity:** evaluates how well the model was able to predict the "negative" class. It's the ratio of correctly classified true negatives to total number of all true negatives or TN/(TN + FP). 

+  **Positive Predictive Value (PPV) or Precision:**  evaluates how well the model was able to predict the "positive" class. It's the ratio of correctly classified true positives to total number of predicted positives or TP/(TP + FP).

+  **Negative Predictive Value (NPV):**  evaluates how well the model was able to predict the "negative" class. It's the ratio of correctly classified true negatives to total number of predicted positives or TN/(TN + FN).
...
+ **K-Nearest Neighbors (KNN):** Uses distance to classify a data point in the test set based upon the most common class of neighboring data points from the training set. For more information on KNN, see [K-Nearst Neighbor](https://www.ibm.com/topics/knn).
...
**Note**: One model is not inherently better than the others with each having their respective advantages and disadvantages. Each algorithm's predictive ability will be largely dependent on the dataset and the situation.  
...
### Changing Data Types 
First, `Detect_Concentration` needs to be converted from a character to a factor, so that Random Forest knows that non-detect data is considered baseline. `Water_Sample_Date` needs to be converted from a character to a date type, so that Random Forest understands this column contains dates.
...
```{r}
# Since the splits in the dataset are random, a seed is set for reproducibility to ensure the splits are occurring
...
Within CV, different parameters will be tested, including the number of decision trees "grown" by RF (`ntree_values`) and the number of predictors used in those trees (`mtry_values`). There is not a formula for choosing the right values to try, however it is recommended to stay closer to 500 decision trees and keeping the number of predictors between 2 and the total number. There are additional paramters that can be tuned, however these will be the only two we'll tune in this example. 

Check out [Random Forests Programming Guide](https://afit-r.github.io/random_forests#tune) for additional information on tuning and building code for RF in R.
...
```{r}
# Need to set the seed again so the RF predictions are consistent
set.seed(12)
...
+ Overall, the model did a decent job at predicting if iAs would be detected based on a balanced accuracy of ~0.6
...
Using the "Manganese_Data", use RF to determine if well water data can be accurate predictors of Manganese detection. The data is structured similarly to the "Arsenic_Data" used in this module, however it now includes 4 additional features:
