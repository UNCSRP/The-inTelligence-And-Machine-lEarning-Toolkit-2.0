...
As presented in the previous training module, machine learning is a field that has been around for decades, but has exploded in popularity and utility in recent years due to proliferation of big data. Through the building of models, machine learning has the ability to sift through and learn from large volumes of data and use that knowledge to solve problems. The challenges of big and high dimensional data as they pertain to environmental health and how machine learning can mitigate some of those challenges are discussed further in [Payton et. al](https://www.frontiersin.org/articles/10.3389/ftox.2023.1171175/full).


Within the field of machine learning, there are many different types of models that can be run to address environmental health research questions. Two major categories of machine learning models commonly used in environmental health research are: (1) supervised machine learning, and (2) unsupervised machine learning.

**Supervised machine learning** involves training a model using a labeled dataset, where each dependent or predictor variable is associated with an independent variable with a known outcome. This allows the model to learn how to best predict the labeled outcome during the model development phase. Once the model is developed and optimized, it can then be used on data it has not "seen" before to predict these outcomes based on the patterns and relationships identified in the data. For example, supervised machine learning has been used for cancer prediction and prognosis based on variables like tumor size, stage, and age ([Lynch et. al](https://www.sciencedirect.com/science/article/abs/pii/S1386505617302368?via%3Dihub), [Asadi et. al](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7416093/)). 

Supervised machine learning includes:

+ Classification models that implement algorithms to classify a categorical outcome (e.g., plant species, disease status, etc.)
+ Regression models that implement algorithms to predict a continuous outcome (e.g., temperature, chemical concentration, etc.)
...
ALEXIS- THIS IS FINE FOR NOW, BUT EVENTUALLY IT WOULD BE GOOD TO RECREATE THIS SIMPLE FIGURE IN BIORENDER SO WE DON'T HAVE COPYRIGHT ISSUES
NOTE THAT WE COULD ASK NOSA TO DO THIS AS SHE REVIEWS THIS MODULE


**Unsupervised machine learning**, on the other hand, involves using models to find patterns or associations between variables in datasets that lack a known or labeled outcome. In some instances, analysts may even decide to exclude outcome/label information to discover different patterns, based upon the question being addressed, and may thus still employ unsupervised machine learning models. For example, unsupervised machine learning has been used to find new groups of genes that act in concerted manners, and thus, could be hypothesized to act along the same or overlapping biological pathways within human cells ([Botía et. al](https://bmcsystbiol.biomedcentral.com/articles/10.1186/s12918-017-0420-6), [Pagnuco et. al](https://www.sciencedirect.com/science/article/pii/S0888754317300575?via%3Dihub)).
...
LET'S ALSO ASK NOSA TO CONTACT THE JOURNAL AND REQUEST PERMISSION TO RE-PRINT THIS FIGURE (WHICH I REALLY LIKE!)
...
ALEXIS WE TYPICALLY USE A 70% / 30% SPLIT IN OUR ANALYSES, COULD WE REVAMP THIS FIGURES TO SHOW THIS SPLIT?

It's typical to split the entire dataset into training and test sets that contain 70% of the data as training and 30% of the data for testing, :
...
Other common splits include 60% training / 40% test and 80% training / 20% test.
...
2. **Validation Set**: a subset of data within the training set (ALEXIS I ADDED THIS IS THIS TRUE?) that is used to evaluate the model's fit in an unbiased way by fine-tuning its parameters and optimizing its performance. This is akin to pop quizzes that help students improve their understanding and performance. 

3. **Test Set:** a subset of data that is used to evaluate the final model's fit based on the training and validation sets. This is akin to the model's final exam, as it provides an objective assessment of the model's ability to generalize to new, unseen data. 
...
The last topic we should mention in this section is **cross validation** or ***k*-fold cross validation**. If the dataset is based on this 70:30 split that we mentioned earlier, our model's accuracy will likely be influenced based upon *where* this 70:30 split occurs in the dataset. Cross validation (CV) is implemented to fine tune a model's parameters and ensure that the model is exposed to more patterns in the dataset, which reduces bias and improves prediction accuracy. 
...
Confusion matrix metrics would be calculated after each iteration and averaged for the final results. For additional information on the different types of cross validation, see: 
...
Metrics from a confusion matrix are used to evaluate model performance for classification-based supervised machine learning models. A confusion matrix consists of table that displays the numbers of how often the algorithm correctly or incorrectly predicted the outcome. 
...
For all metrics, the values fall between 0 and 1, where 0 indicates that the model was not able to classify any data points correctly and 1 indicating that the model was able to classify all test data correctly. Typically a balanced accuracy of at least 0.7 is considered respectable. ALEXIS DO YOU HAPPEN TO HAVE A REFERENCE FOR THE 0.7 CUT-OFF?
...
Although this module's example focuses on a random forest model below, other notable algorithims commonly used in supervised machine learning exercises include: 
...
ALEXIS CAN WE PLEASE INSERT NOSE'S TREE FIGURE HERE? I REALLY LIKE IT


Here is a published example decision tree with potential variables and decisions informing low vs high risk of having a heart attack:
...
For more information on RF, see ([Navlani, 2023](https://www.datacamp.com/tutorial/decision-tree-classification-python)); and on decision trees, see [Decision Trees](https://www.ibm.com/topics/decision-trees#:~:text=A%20decision%20tree%20is%20a,internal%20nodes%20and%20leaf%20nodes)
...
In this activity, we will analyze an example dataset to see whether we can use environmental monitoring information to predict areas of contamination through random forest modeling. Specifically, this supervised machine learning method will be applied to a dataset of well water descriptive variables, spanning geospatial location, sampling date, and well water attributes, with the goal of predicting whether detectable levels of inorganic arsenic (iAs) are present. This dataset was obtained through the sampling of 713 private wells across North Carolina using a method that was capable of detecting levels of iAs > 10 ppm (ALEXIS IS THIS CORRECT THAT WE USED THE 10 PPM CUT-OFF HERE? OR LOWER?). After the model is built, we walk through different parameters that are used to assess model performance.
...
1. Which well water descriptive variables, spanning geospatial location, sampling date, and well water attributes, significantly differ between samples containing detectable levels of iAs vs samples that are not contaminated?

2. How can we train a random forest (RF) model to predict whether a well water location might be contaminated with inorganic arsenic (iAs)?

3. How can we evaluate the performance of this RF model?

4. With this RF model, can we predict if iAs will be detected based on well water information? 
...
It is useful to run summary statistics on the variables that will be used as predictors in the model to see if there are differences in distributions between the outcomes classes (either non-detect or detect in this case). Typically, greater significance often leads to better predictivity for a certain variable, since the model is better able to separate the classes. We'll use the `tbl_summary()` function from the `gtsummary` package.
...
All the variables are very significant with the exception of the sample date and the static water depth, therefore the model should predict fairly well.
...
For additional learning materials on machine learning models, see the following resources: 

**I WILL LIKELY CHANGE THESE RESOURCES...I THINK WE CAN FIND BETTER ONES** ALEXIS- YES AGREED, LET'S ALSO ASK OTHERS FOR THEIR FAVORITES - KYLE HAS RESPONDED WITH SOME GOOD SUGGESTIONS AT THIS POINT
...
I LOVE KEEPING IT SIMPLE - LET'S USE THE CHROMIUM DATASET WITH THE SAME STRUCTURE!!
