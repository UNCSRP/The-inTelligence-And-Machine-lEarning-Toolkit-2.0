...
+ Do not include special characters, such as $, #, @, !, %, &, *, (, ), and +. Special characters tend to generate errors on local harddrives when syncing to cloud-based servers, and they are difficult to upload into programming software.
...
1. Try wrangling the "ExcelWrangling_TYKRawDataset.xlsx" to mimic the cleaned versions of the data found in "ExcelWrangling_TYKWrangledDataKey.xlsx". This dataset includes sterol and cytokine concentration levels extracted from induced sputum samples collected after ozone exposure. After wrangling, you should end up with a sheet for subject information and a sheet for experimental data.
